...
df_clean_final <- combined |>
  filter(!is.na(created_date), !is.na(complaint_type), !is.na(borough))
print(before_n - nrow(df_clean_final, ...)
)
# 3) drop rows with invalid resolution ordering (closed before created) but keep NA closed
before_n <- nrow(df_clean_final)
df_clean_final <- df_clean_final |>
  filter(is.na(closed_date) | is.na(created_date) | closed_date >= created_date)
cat("Dropped due to invalid time ordering:", before_n - nrow(df_clean_final), "\n")
