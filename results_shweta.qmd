...
###  Borough-level complaint patterns

#### Borough × complaint bucket summary table

We started our further interpretation by making borough–by–complaint-bucket (50 rows).  
Each row combines:
- a borough  
- one of our merged **complaint buckets** (Noise Related, Vehicle and Parking, etc.)  
- the **count** in that bucket  
- and the **total** complaints for that borough.

This table is the **numerical base layer** for the rest of the results.It lets us double-check that proportions later on come from sensible counts, and it makes clear that the comparisons are always “within borough”.
...
#### Cleveland dot plot: complaint categories by borough
...
- Faceting each borough into its own panel keeps labels readable.
...
### Complaint flow from year to closure status

#### Alluvial diagram: Year → Complaint Type → Final Status
...
```
- We are interested in a **process**, not just separate counts: where do complaints go?  
- A simple table of year × type × status would be hard to read; an alluvial diagram shows how volume splits and rejoins across stages.  
- It visually emphasises **which types are more likely to end up stuck** in “Assigned” or “In Progress”.
...
```
### 3.3 System stress: backlog over time
...
```
- Backlog is a single time-series; we mainly care about its **trend and scale**.  
- A filled area conveys the idea of “work piling up” better than a thin line, without
...
### How people contact 311: channels by time of day

#### Faceted line plot: submission channels by shift

We then turned to **how** people submit complaints: ONLINE, PHONE, MOBILE, and OTHER.For each time-of-day shift (Late Night, Morning, Afternoon, Evening), we calculated each channel’s **share of monthly complaints** and plotted these as **line charts in a 2×2 facet layout**.
...
```
- Channel use changes over time *and* depends on time of day.  
- Faceting by shift lets us keep a consistent y-axis and legend but separates patterns
that would otherwise overlap.  
- Using **shares instead of counts** means we are comparing composition, not total
volume, which would be dominated by day-time hours.
...
### 3.5 Who handles what: interactive borough–type–agency “pair” plot
...
### 3.6 Where problems occur and what gets done

#### 3.6.1 Heatmap: complaint composition by location type
...
#### 3.6.2 Resolution actions by location type (faceted bar charts)
...
Together, these panels close the loop from **complaint location** to **typical outcome**.



```{r label: timeline-by-bucket,fig-width: 10,fig-height: 6}

library(dplyr)
library(tidyr)
library(ggplot2)

timeline <- df_clean_final |>
  mutate(
    created_date = as.Date(created_date),
    action_date  = as.Date(resolution_action_updated_date),
    closed_date  = as.Date(closed_date),
    t_to_action       = as.numeric(difftime(action_date, created_date, units = "days")),
    t_action_to_close = as.numeric(difftime(closed_date, action_date, units = "days"))
  ) |>
  filter(
    !is.na(t_to_action),
    !is.na(t_action_to_close),
    t_to_action       >= 0,
    t_action_to_close >= 0,
    t_to_action       <= 60,
    t_action_to_close <= 60
  ) |>
  group_by(complaint_bucket) |>
  summarise(
    median_to_action       = median(t_to_action),
    median_action_to_close = median(t_action_to_close),
    .groups = "drop"
  ) |>
  mutate(
    total_median = median_to_action + median_action_to_close,
    complaint_bucket = reorder(complaint_bucket, total_median)
  )

timeline_long <- timeline |>
  select(complaint_bucket, median_to_action, median_action_to_close) |>
  pivot_longer(
    cols = c(median_to_action, median_action_to_close),
    names_to = "phase",
    values_to = "days"
  )

ggplot(timeline_long,
       aes(x = complaint_bucket, y = days, fill = phase)) +
  geom_col() +
  coord_flip() +
  scale_fill_manual(
    values = c("median_to_action" = "#1b9e77",
               "median_action_to_close" = "#7570b3"),
    labels = c("Time to first action", "Time from action to closure"),
    name = NULL
  ) +
  labs(
    title = "Where Is Time Spent? Decomposing Complaint Lifecycles by Type",
    subtitle = "Bars show median days; darker section is waiting for the first recorded action,\nlighter section is time from action to full closure (within 60-day window).",
    x = "Complaint bucket",
    y = "Median days"
  ) +
  theme_report()
