...
```
## How does each department perform in resolving complaints?
```{r}
dept_performance <- df_clean_final |>
  group_by(agency_name, status) |>
  summarise(count = n(), .groups = "drop") |>
  group_by(agency_name) |>
  mutate(
    total_agency = sum(count),
    proportion = count / total_agency * 100
  ) |>
  ungroup()
# Stacked bar plot of department performance
ggplot(dept_performance, aes(x = fct_reorder(agency_name, total_agency), y = proportion, fill = status)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  labs(
    title = "Department Performance in Resolving Complaints",
    x = "Agency",
